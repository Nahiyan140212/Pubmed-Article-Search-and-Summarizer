...
def fetch_pubmed_count(query, sort_option="relevance"):
    ...
    search_params = {
        ...
        "retmode": "json",
        "usehistory": "y",
        "sort": sort_option  # Add sort parameter
    }
...
with st.sidebar:
    ...
    with st.form(key='search_form'):
        ...
        sort_option = st.selectbox("Sort Results By", ["Best Match", "Most Recent"])  # New sorting option
        ...
        for i, history_item in enumerate(st.session_state.search_history[-5:]):
            if st.button(f"{history_item[:40]}...", key=f"history_{i}"):
                ...
                with st.spinner("Searching PubMed..."):
                    # Use the same sort option as last selected
...
if search_button:
    ...
    if query:
        ...
        with st.spinner("Searching PubMed..."):
            # Map sort_option to PubMed API values
            sort_param = "relevance" if sort_option == "Best Match" else "date"
